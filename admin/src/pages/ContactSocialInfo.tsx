...
const ContactSocialInfo = () => {
  const [fields, setFields] = useState<InfoField[]>([
    { id: 'whatsapp', label: 'WhatsApp', value: '44 028278212', icon: Phone, type: 'contact' },
    { id: 'email', label: 'Email address', value: 'crystalclaire@gmail.com', icon: Mail, type: 'contact' },
    { id: 'address', label: 'Address', value: 'Non', icon: MapPin, type: 'contact' },
    { id: 'instagram', label: 'Instagram', value: '@aprosphere', icon: Instagram, type: 'social' },
    { id: 'tiktok', label: 'Tiktok', value: '@aprosphere', icon: Music2, type: 'social' },
  ...
  const handleSave = async () => {
    if (editingField) {
      try {
        ...
        setFields(fields.map(f => f.id === editingField.id ? { ...f, value: newValue } : f));
  ...
  if (loading) return (
    <div className={listStyles.container} style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '60vh' }}>
      ...
      <div className={styles.container}>
        <div className={listStyles.pageHeader}>
          <div className={listStyles.headerLeft}>
            <h1 className={listStyles.pageTitle} style={{ color: 'var(--text-main)' }}>Contact & Social Info</h1>
          </div>
        </div>
        <section className={styles.section}>
          ...
          <div className={styles.grid}>
            {contactFields.map((field) => (
              <div key={field.id} className={styles.card} onClick={() => handleEdit(field)}>
        ...
        <div className={styles.modalOverlay} onClick={() => setEditingField(null)}>
          <div className={styles.modalWrapper} onClick={(e) => e.stopPropagation()}>
            <div className={styles.modalDetailCard}>
              <button className={styles.closeButton} onClick={() => setEditingField(null)}>
                <X size={20} />
              </button>
              <div className={styles.modalHeader}>
                ...
                <h3 className={styles.modalTitle}>Update {editingField.label}</h3>
                <p style={{ color: 'var(--text-light)', marginTop: '8px', fontSize: '0.95rem' }}>
                  Enter the new information for your {editingField.label.toLowerCase()}.
                </p>
              </div>
